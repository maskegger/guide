...
<img src="https://github.com/reifjulian/guide/blob/master/assets/guide/stata_profile.PNG" width="100%" title="Stata profile">
...
An analysis starts with raw data (e.g., a dataset downloaded from the web). Scripts process these data, run analyses, and create tables. So the minimal folder structure for a project looks like this:
...
Many people do not appreciate how code updates can inhibit replication. Here is an example. You perform a Stata analysis using a new, user-written estimation command called, say, `regols`. You publish your paper, along with your replication code, but do not include the code for `regols`. Ten years later a researcher tries to replicate your analysis. The code breaks because she has not installed `regols`. She opens Stata and type `ssc install regols`, which installs the newest version of that command. But, in the intervening ten years the author of `regols` fixed a bug in how the standard errors are calculated. When the researcher runs your code she finds your estimates are no longer statistically significant. Is this because you included the wrong dataset with your replication, because there is mistake in the analysis code, or because you failed to correctly copy/paste your output into your publication? The researcher does not know. She cannot replicate your published results and must now decide what to do.

Stata takes version control [seriously](https://www.stata.com/features/integrated-version-control/). At a minimum, you should always include a `version` statement in the final version of your published code. Unfortunately, many user-written packages (including my own) are not carefully version controlled.  When I start a new project, I include a script called [_install_stata_packages.do](https://github.com/reifjulian/my-project/blob/master/MyProject/analysis/scripts/_install_stata_packages.do) that installs a copy of any user-written packages I need for the project into a subdirectory of the project folder: **analysis/scripts/libraries/stata**. Rerunning this script will install updated versions of these add-on's (if desired). I delete the script when my project is ready to be published, which locks down the code for these user-written packages and thus ensures I can exactly replicate my Stata analysis forever.

I am unaware of a version control statement for **R**. Instead, my replication package contains a short script, [_confirm_version.R](https://github.com/reifjulian/my-project/blob/master/MyProject/analysis/scripts/functions/_confirm_version.R), which checks whether the user: (1) is running a sufficiently recent version of **R**; and (2) has installed [tidyverse](https://tidyverse.tidyverse.org/), a standard set of **R** packages. When working with **R**, I include a similar script called [_install_R_packages.R](https://github.com/reifjulian/my-project/blob/master/MyProject/analysis/scripts/_install_R_packages.R).

If replication of the **R** code is very important, you may want to look into [packrat](https://rstudio.github.io/packrat/).
